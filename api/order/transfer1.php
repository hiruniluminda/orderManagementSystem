...
if(isset($_POST['accept_button'])) {
    ...
    $sql_select_order = "SELECT * FROM users WHERE inv_id = $order_id";
    ...
    if ($result_select_order->num_rows > 0) {
        ...
        $sql_delete_order = "DELETE FROM users WHERE inv_id = $order_id";
        if ($conn->query($sql_delete_order) === TRUE) {
            // Insert the order into checks table
            $sql_insert_check = "INSERT INTO not_checked_received (inv_id, name, email, mobile) VALUES ($inv_id,'$name','$email',$mobile)";
...
$sql_select_orders = "SELECT * FROM users";
